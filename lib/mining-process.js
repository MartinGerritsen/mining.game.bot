...
module.exports = class MiningProcess {
  ...
  async sendReport(report) {
    if (process.env.DISCORD) {
      const hook = new Webhook(
        `https://discord.com/api/webhooks/${process.env.DISCORD}`
      );

      if (hook) {
        await hook.send(report);
      }
    }
  }
  ...
  async runProcess() {
    ...
    if (!this.lastReport || now - this.lastReport > DayInMilliseconds) {
      this.lastReport = now;
      await this.sendReport(`${this.network}\n${information}`);
    }
  ...
  async gatherInformation() {
    ...
    const information = `Pending WATT: ${pendingWatt.toFixed(
      2
    )}\nPending value: $${(
      pendingWatt * Number(this.wattPair?.priceUsd || 0)
    ).toFixed(2)}\nWATT price: $${Number(
      this.wattPair?.priceUsd || 0
    )}\nChange 24h: ${priceChange}`;
